...
We use [labels](https://github.com/flarebuild/rules_idea/labels) for the
...
from the [flarebuild/buildtools](https://github.com/flarebuild/buildtools) to
format `BUILD` and `*.bzl` files. We also use `buildifier --lint=warn` to check
for common issues.

You can download `buildifier` from
[flarebuild/buildtools Releases Page](https://github.com/flarebuild/buildtools/releases).
